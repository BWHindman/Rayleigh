...
files = build_file_list(iter0,iter1,path='G_Avgs')
print(files)
...
# BWH 2/1/2018: This part of the code has been modified from Nick's original to permit the last file
#	in the list to have a different number of iteration
nfiles = len(files)
for i,f in enumerate(files):
	...
	i1 = 0
    i0 = i1				# BWH 2/7/2018: This line has been modified to accommodate files with fewer iterations
    i1 = i0 + a.niter  		        # BWH 2/1/2018: This line has been modified to accomodate files with fewer iterations
    gavgs[i0:i1,:] = a.vals
    time[i0:i1] = a.time
    iters[i0:i1] = a.iters
gavgs = gavgs[0:i1,:]				# BWH 2/1/2018: The next three lines have been added
time = time[0:i1]/(3600.0*24.0*365.2422)	# 		Convert to years
iters = iters[0:i1]
...
sizetuple=(16,14)
fig, ax = plt.subplots(nrows=2, ncols=2, figsize=sizetuple)
ax[0,0].plot(time, gavgs[:,ke], label='KE')
ax[0,0].plot(time, gavgs[:,mke],label='MKE')
ax[0,0].plot(time, gavgs[:,fke], label='FKE')
ax[0,0].legend(loc='center right', shadow=True)
ax[0,0].set_xlim([0,0.2])
ax[0,0].set_title('Equilibration Phase')
ax[0,0].set_xlabel('Time {years}')
ax[0,0].set_ylabel('Energy')
ax[0,0].set_yscale('log')

ax[0,1].plot(time, gavgs[:,ke], label='KE')
ax[0,1].plot(time, gavgs[:,mke], label = 'MKE')
ax[0,1].plot(time,gavgs[0:,fke],label='FKE')
ax[0,1].legend(loc='center right', shadow=True)
ax[0,1].set_title('Entire Time-Trace')
ax[0,1].set_xlabel('Time {years}')
ax[0,1].set_ylabel('Energy')
ax[0,1].set_yscale('log')

ax[1,0].plot(iters, gavgs[:,ke], label='KE')
ax[1,0].plot(iters, gavgs[:,mke],label='MKE')
ax[1,0].plot(iters, gavgs[:,fke], label='FKE')
ax[1,0].legend(loc='center right', shadow=True)
ax[1,0].set_xlim([0,10000])
ax[1,0].set_title('Equilibration Phase')
ax[1,0].set_xlabel('Iterations')
ax[1,0].set_ylabel('Energy')
ax[1,0].set_yscale('log')

ax[1,1].plot(iters, gavgs[:,ke], label='KE')
ax[1,1].plot(iters, gavgs[:,mke], label = 'MKE')
ax[1,1].plot(iters, gavgs[0:,fke],label='FKE')
ax[1,1].legend(loc='center right', shadow=True)
ax[1,1].set_title('Entire Time-Trace')
